...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
    void refreshPage0() {
      double pp;
     float kw_val = power_mw_pep/1000000;
  float W_val  = power_mw_pep/1000;
  float mW_val = power_mw_pep/1;
  float nW_val = power_mw_pep*1000;
   if(power_mw_pep <1000000 and power_mw_pep >= 500000){    
   pp=kw_val/100+0.1;
   }
  if(power_mw_pep <500000 and power_mw_pep >= 10000){    
   pp=W_val/100+0.1;
   }
   if(power_mw_pep <1000 and power_mw_pep >= 1){    
   pp=mW_val/100+0.1;
   }
   if(power_mw_pep <1 ){    
   pp=nW_val/100+0.1;
      ...
      nex.writeStr("sn.txt", coupler_name);
      nex.writeNum("pp.val",pp);
...
void trigger13(){
current_coupler =1;
coupler_name= nex.readStr("t5.txt");
...
void trigger14(){
 current_coupler= 2;
coupler_name= nex.readStr("t6.txt");
}
...
void trigger15(){
  current_coupler= 3;
coupler_name= nex.readStr("t7.txt");
}
...
void trigger16(){
 current_coupler= 4; 
coupler_name= nex.readStr("t8.txt");
